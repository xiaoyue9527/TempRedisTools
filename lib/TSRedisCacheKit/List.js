...
class CacheList extends base_1.BaseCache {
    /**
     * 从右侧推入元素到列表
     * @param suffix - 可选的键后缀
     * @param values - 要推入的元素
     ...
     * @returns 设置结果
     */
    async set(index, value, suffix = "") {
        return await this.redis.lset(this.createKey(suffix), index, this.dataToString(value));
    }
    /**
     * 根据值移除元素
     * @param count - 移除的元素数量
     ...
     * @returns 移除的元素数量
     */
    async remove(count, value, suffix = "") {
        return await this.redis.lrem(this.createKey(suffix), count, this.dataToString(value));
    }
    /**
     * 在指定元素前或后插入新元素
     * @param pivot - 参考元素
     ...
     * @returns 插入后列表的长度
     */
    async insert(pivot, value, position, suffix = "") {
        ...
        if (position === "BEFORE") {
            ...
            throw new Error("Invalid position argument. Must be 'BEFORE' or 'AFTER'.");
        }
    }
}
