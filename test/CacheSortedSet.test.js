...
describe("CacheSortedSet", () => {
    ...
    beforeEach(() => {
        ...
        cacheSortedSet = new SortedSet_1.CacheSortedSet("testPrefix", cacheOption, redisClient);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    test("should add a member to sorted set", async () => {
        ...
        expect(redisClient.zadd).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 1, "value");
    ...
    test("should add multiple members to sorted set", async () => {
        ...
        expect(redisClient.zadd).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 1, "value1", 2, "value2");
        expect(result).toBe(true);
    });
    test("should remove a member from sorted set", async () => {
        const result = await cacheSortedSet.remove("value");
        expect(redisClient.zrem).toHaveBeenCalledWith("testPrefix-testApp-testFunc", "value");
        expect(result).toBe(true);
    });
    test("should count members in score range", async () => {
        const result = await cacheSortedSet.count(1, 2);
        expect(redisClient.zcount).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 1, 2);
        expect(result).toBe(2);
    });
    test("should get members in score range", async () => {
        const result = await cacheSortedSet.rangeByScore(1, 2);
        expect(redisClient.zrangebyscore).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 1, 2);
        expect(result).toEqual(["value1", "value2"]);
    });
    test("should get sorted set length", async () => {
        const result = await cacheSortedSet.length();
        expect(redisClient.zcard).toHaveBeenCalledWith("testPrefix-testApp-testFunc");
        expect(result).toBe(2);
    });
    test("should get members in range", async () => {
        const result = await cacheSortedSet.range(0, 1);
        expect(redisClient.zrange).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 0, 1);
        expect(result).toEqual(["value1", "value2"]);
    });
    test("should get members with scores in range", async () => {
        const result = await cacheSortedSet.rangeWithScores(0, 1);
        expect(redisClient.zrange).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 0, 1, "WITHSCORES");
    ...
    test("should get score of a member", async () => {
        ...
        expect(redisClient.zscore).toHaveBeenCalledWith("testPrefix-testApp-testFunc", "value");
        expect(result).toBe(1.0);
    });
    test("should increment score of a member", async () => {
        const result = await cacheSortedSet.incrementBy("value", 1);
        expect(redisClient.zincrby).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 1, "value");
        expect(result).toBe("2.0");
    });
    test("should remove members in score range", async () => {
        const result = await cacheSortedSet.removeRangeByScore(1, 2);
        expect(redisClient.zremrangebyscore).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 1, 2);
        expect(result).toBe(1);
    });
    test("should remove members in rank range", async () => {
        const result = await cacheSortedSet.removeRangeByRank(0, 1);
        expect(redisClient.zremrangebyrank).toHaveBeenCalledWith("testPrefix-testApp-testFunc", 0, 1);
        expect(result).toBe(1);
    });
});
